...
from kinamtic_1_1 import rotation_matrices
...
class RRT:
    ...
    def midpoint(self, startp, endp):
        return np.linspace(startp, endp, int(np.max(np.abs(startp - endp) + 1)),dtype=np.float32)
...
# =============================================================================
    ...
    def let_the_magic_begin(self, ros_fun,isflipt):
        ...
        while (not self.statos.value):
            ...
            newpoint =  (np.float32(np.random.rand(1, 12,)) * self.kin.limit * self.size - self.kin.offset)
...
if __name__ == '__main__':
    ...
    end = np.array([[0, 0, -90, 0, 0, 0, 0, 0, -90, 0, 0, 0]],dtype=np.float32)
    #(arm,-arm)
    #pos = np.array([50, 100, 400, -50,100, 400])
    pos = np.array([100, 0,100,-100,0, 100],dtype=np.float32)
    #pos = np.array([211,125, 350,-112,0, 378])

    ori = np.array([ 0, np.pi/2, 0, 0,  np.pi/2, 0],dtype=np.float32)
    #ori = np.array([0, 0, 0, 0, 0, 0])

    ros_meneger = ros(100)
    start = np.float32(ros_meneger.readstat())
